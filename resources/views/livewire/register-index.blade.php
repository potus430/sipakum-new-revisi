<div class="p-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="flex gap-2 w-full md:w-auto">
            ...
            <flux:input wire:model.live="search" icon="magnifying-glass" placeholder="Cari nomor/nama..."
                class="flex-1" />
        </div>
    </div>
                ...
                <flux:table.rows>
                    @forelse ($registers as $item)
                        <flux:table.row>
                            ...
                            {{-- Ganti isi Cell Pihak Terkait di register-index.blade.php --}}
                            <flux:table.cell>
                                <div class="flex flex-col">
                                    <span class="font-medium text-zinc-900 dark:text-white">
                                        {{-- Menampilkan kolom subjek yang sudah mencakup semua pihak --}}
                                        {{ $item->subjek ?? '-' }}
                                    </span>

                                    <div class="flex gap-2 mt-1">
                                        {{-- Label Modul untuk memperjelas --}}

                                        @if (isset($item->metadata['jenis_perkara']))
                                ...
                                <div class="flex -space-x-2">
                                    @foreach ($item->files->take(3) as $file)
                                        <div title="{{ $file->file_name }}"
                                            class="w-7 h-7 rounded-full bg-zinc-100 border border-white flex items-center justify-center">
                                            <flux:icon name="document-text" size="xs" class="text-zinc-400" />
                                        </div>
                                    @endforeach
                                    @if ($item->files->count() > 3)
                                        <div
                                            class="w-7 h-7 rounded-full bg-zinc-200 border border-white flex items-center justify-center text-[10px] font-bold">
                                            +{{ $item->files->count() - 3 }}
                                        </div>
                                    @endif
                                </div>
